...
* `allow_fallback_ifcs` - when Clearwater cannot find an Identity element in the user profile XML from the HSS that matches the served user, it will normally not invoke any services. When this is set to Y, it will use the first set of Initial Filter Criteria in the profile (giving the ability to work around HSSes that return something unexpected, or the ability to get basic support for wildcard public service identities).
