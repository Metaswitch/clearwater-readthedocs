...
* `sprout_hostname_mgmt` - a hostname that resolves by DNS round-robin to the management interface of all Sprout nodes in the cluster.  Should include the HTTP port (always 9886).
...
* `hs_hostname_mgmt` - a hostname that resolves by DNS round-robin to the management interface of all Homestead nodes in the cluster.  Should include the HTTP port (always 8886).
