...
* `non_register_authentication` - controls when Sprout will challenge a non-REGISTER request using SIP Proxy-Authentication. This option is a comma separated list that may contain the values listed below (e.g. `non_register_authentication=if_proxy_authorization_present,initial_request_from_req_digest_endpoint`):
...
* `sprout_impi_store` - this is the location of Sprout's IMPI store. It just has the same format as `sprout_registration_store`. If not provided, Sprout uses the same value configured in `sprout_registration_store`.
