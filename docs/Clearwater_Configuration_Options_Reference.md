...
* `pbxes` - a comma separated list of IP address that Bono considers to be PBXs that are incapable of registering. Non-REGISTER requests from these addresses are passed upstream to Sprout with a `Proxy-Authorization` header that causes Sprout to challenge it. Bono also permits requests to these addresses from the core to pass through it.
* `non_register_authentication` - controls when Sprout will challenge a non-REGISTER request using SIP Proxy-Authentication. Possible value are `never` (meaning Sprout will never challenge) or `if_proxy_authorization_present` (meaning Sprout will only challenge requests that have a Proxy-Authorization header). 
