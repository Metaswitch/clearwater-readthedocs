...
@@ talk about signaling, management, private IP addresses
...
You can check the key fingerprint with:

    sudo apt-key finger

which should produce output containing the following:
...
These instructions reference two optional concepts:
...
Create the file `/etc/clearwater/local_config` with the following contents.

    local_ip=<private signaling IP>
    public_ip=<public signaling IP>
    public_hostname=<hostname resolving to public_ip>
    etcd_cluster="<comma separated list of private IPs>"
    signaling_namespace=<namespace name> # optional
    signaling_dns_server=<DNS IP address> # optional
    management_local_ip=<management IP> # optional
    local_site_name=<site> # optional
    remote_site_name=<site> # optional

Create the file `/etc/chronos/chronos.conf` with the following contents:

    [http]
    bind-address = <private signaling IP>
    bind-port = 7253
    threads = 50
...
Install the Homer package with:
...
Install the Bono packages with:
