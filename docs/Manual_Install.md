...
* `local_site_name` and `remote_site_name` should reflect the node's location (e.g. a node in site A
  should have `local_site_name=siteA` and `remote_site_name=siteB`, whereas a node in site B should
  have  `local_site_name=siteB` and `remote_site_name=siteA`)
