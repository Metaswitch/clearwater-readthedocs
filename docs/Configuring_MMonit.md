...
When creating a instance pass it the mmonit hostname in `/etc/clearwater/shared_config`, eg: mmonit_hostname=mmonit.example.com. To configure an existing instance without this in the user data, simply stop the instance, modify the file and start it again.
