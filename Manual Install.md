...
* SSH access to the above machines to a user called `ubuntu` authorised to use sudo.  If your system does not come with such a user pre-configured, follow the instructions [here](https://help.ubuntu.com/community/RootSudo#Allowing_other_users_to_run_sudo) to give a non-root user sudo permission.
